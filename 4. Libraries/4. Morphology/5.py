...
for item in text:
    ...
    for word in item:
        ...
        if 'Name' in word_morphed.tag:
            names.add(word_morphed.inflect({'nomn'}).word.capitalize())
            continue
        if 'anim' in morph.parse(word)[0].tag:
            if word_morphed.inflect({'nomn', 'sing'}):
                anims.add(word_morphed.inflect({'nomn', 'sing'}).word)
                continue
